...
from src.miband_constants import UUIDS, AUTH_STATES, ALERT_TYPES, QUEUE_TYPES, MUSICSTATE
import struct
...
class miband(Peripheral):
    ...
    def start_accel_realtime(self, accel_measure_callback):
        self.gyro_avg_callback = accel_measure_callback

        self.start_accel_request()
        t = time.time()
        while True:
            self.waitForNotifications(0.5)
            self._parse_queue()
    ...
    def start_accel_request(self):
        ...
        char_sensor.write(b'\x01\x01\x19', withResponse=False)
        self.writeCharacteristic(sensor_handle, b'\x00\x00', withResponse=True)
        char_sensor.write(b'\x02', withResponse=False)
